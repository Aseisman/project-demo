...
export default {
  ...
  methods: {
    ...
    // 处理滚动事件
    handleScroll (shouldUpdate = true,ignoreHorizontalScroll = false) {
      // 横向滚动不处理 start
      if(!ignoreHorizontalScroll){
        var afterScrollTop = document.body.scrollTop,
        delta = afterScrollTop - beforeScrollTop;
        if (ignoreHorizontalScroll && delta === 0) return false;
